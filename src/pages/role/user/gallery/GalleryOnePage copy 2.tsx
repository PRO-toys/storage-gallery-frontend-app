...
// ======================================================
// GalleryOnePage — with polling, queue gate, cache-busting, and CardShareSocial
// ======================================================
...
const GalleryOnePage: React.FC = () => {
    ...
    if (event_code && qrcode) {
      ...
      const fetchQueueStatus = async () => {
        try {
          ...
          if (data?.status === 'success' && data?.exists && data?.data?.queue_status) {
          ...
          // If status changed, update and refresh gallery
          if (nextStatus !== prevQueueStatusRef.current) {
          ...
          // Treat as error; if changed, refresh gallery
      ...
      // Poll ONLY the queue endpoint; gallery refetches on status changes
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center px-4 py-10">
      ...
      {/* ======================= */}
      {/* Queue Gate — Progress  */}
      {/* Show only when queue is pending or error */}
      {/* ======================= */}
      <CardProgressRandom
        show={queueStatus === 'pending' || queueStatus === 'error'}
        refreshToken={`${event_code}-${qrcode}-${queueStatus ?? 'none'}`}
        gifWidthPx={300}
      ...
      {/* ======================= */}
      <CardShareSocial
        // pageUrl="https://storage-gallery-backend.protoys.online/storage/gallery/DEV/A001/RenderImage1.jpg?t=1757397508073" //example comment
