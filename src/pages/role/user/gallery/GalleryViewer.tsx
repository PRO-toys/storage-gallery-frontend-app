...
const GalleryViewer: React.FC = () => {
  ...
  const handleToggleLive = () => setLiveFeed((v) => !v);
  const handleManualRefresh = () => fetchGallery();
  ...
  return (
    ...
    >
      <div className="w-full max-w-6xl flex items-center justify-between mb-4">
        <img src={logoSocialhub} alt="Socialhub Logo" className="w-[200px] md:w-[300px] h-auto" />
        <div className="flex items-center gap-6">
          <button
            onClick={handleManualRefresh}
            aria-label="Refresh gallery"
            title="Refresh"
            className="p-1 rounded-lg bg-transparent hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-white/70 active:scale-95 transition"
          >
            <img
              src={iconRefresh}
              alt="Refresh"
          ...
          <button
            ...
            aria-label={liveFeed ? "Turn live feed OFF" : "Turn live feed ON"}
            title={liveFeed ? "Live: ON (click to turn OFF)" : "Live: OFF (click to turn ON)"}
            aria-pressed={liveFeed}
            className="p-1 rounded-lg bg-transparent hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-white/70 active:scale-95 transition"
          >
            <img
              src={liveFeed ? iconLiveOn : iconLiveOff}
              alt={liveFeed ? "Live ON" : "Live OFF"}
              className="w-16 h-16 md:w-20 md:h-20 select-none pointer-events-none"
              draggable={false}
            />
          </button>
      ...
      <div className={`flex flex-col md:flex-row justify-center items-center gap-6 w-full max-w-[1000px] ${gapBelowViewer}`}>
        ...
        <div className="flex-1 max-w-[350px] text-center">
          <p className="font-bold mb-2 text-white drop-shadow">EXTREME AI PHOTO</p>
          {selectedMedia?.qrcodeUrl ? (
            <>
              ...
              {qrPublicLink && (
                <a
                  ...
                  className="mt-3 inline-block text-xs md:text-sm text-blue-200 hover:text-white underline break-all"
                >
                  {selectedMedia.qrcode}
                </a>
              )}
            ...
            <p className="text-gray-200">No QR Code available</p>
          )}
        </div>
      </div>
